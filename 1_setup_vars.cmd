...
if "%PRINTERBRAND%" == "" (
    ...
    set PRINTERBRAND_DEFAULT_VALUE=%PRINTERBRAND%
)

if "%PRINTERMODEL%" == "" (
    set PRINTERMODEL_DEFAULT_VALUE=iX6850
) else (
    set PRINTERMODEL_DEFAULT_VALUE=%PRINTERMODEL%
)

if "%PAPERBRAND%" == "" (
    set PAPERBRAND_DEFAULT_VALUE=Kodak
) else (
    set PAPERBRAND_DEFAULT_VALUE=%PAPERBRAND%
)

if "%PAPERMODEL%" == "" (
    set PAPERMODEL_DEFAULT_VALUE=UPPP
) else (
    set PAPERMODEL_DEFAULT_VALUE=%PAPERMODEL%
)

if "%PAPERFINISH%" == "" (
    set PAPERFINISH_DEFAULT_VALUE=Glossy
) else (
    set PAPERFINISH_DEFAULT_VALUE=%PAPERFINISH%
)

if "%PAPERSIZE%" == "" (
    set PAPERSIZE_DEFAULT_VALUE=A4
) else (
    set PAPERSIZE_DEFAULT_VALUE=%PAPERSIZE%
)

if "%INK_BRAND%" == "" (
    set INK_BRAND_DEFAULT_VALUE=Canon
) else (
    set INK_BRAND_DEFAULT_VALUE=%INK_BRAND%
)

if "%USE_HIGH_DENSITY_MODE%" == "" (
    set USE_HIGH_DENSITY_MODE_DEFAULT_VALUE=True
) else (
    set USE_HIGH_DENSITY_MODE_DEFAULT_VALUE=%USE_HIGH_DENSITY_MODE%
)

if "%NUMBER_OF_PAGES%" == "" (
    set NUMBER_OF_PAGES_DEFAULT_VALUE=1
) else (
    set NUMBER_OF_PAGES_DEFAULT_VALUE=%NUMBER_OF_PAGES%
)

if "%PER_PAGE_PATCH_COUNT%" == "" (
    set PER_PAGE_PATCH_COUNT_DEFAULT_VALUE=
) else (
    set PER_PAGE_PATCH_COUNT_DEFAULT_VALUE=%PER_PAGE_PATCH_COUNT%
)

if "%COPYRIGHT%" == "" (
    set COPYRIGHT_DEFAULT_VALUE=
) else (
    set COPYRIGHT_DEFAULT_VALUE=%COPYRIGHT%
)
...
set /p COPYRIGHT=Copyright Info? (%COPYRIGHT_DEFAULT_VALUE%)
if "%COPYRIGHT%"=="" set COPYRIGHT=%COPYRIGHT_DEFAULT_VALUE%
